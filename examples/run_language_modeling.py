...
# TODO: this function not necessary; just put into main(); easier to read
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.per_gpu_train_batch_size, collate_fn=collate)

    if args.max_steps > 0:
        num_instances_in_dataset = len(train_dataloader)
        num_train_epochs = args.max_steps // (num_instances_in_dataset // args.gradient_accumulation_steps) + 1
    else:
        raise Exception(f'Please specify --max_steps > 0')
    ...
    warmup_steps = int(args.warmup_pct * args.max_steps)
    ...
    logger.info(f"""
       ...
       True effective batch size of {true_effective_batch_size}.
       
       Since our dataset has N = {num_instances_in_dataset} instances in total, 
       and we want to perform {args.max_steps} gradient updates,
       we'll need to make (approx) {num_train_epochs} epochs through the entire dataset.
    """)
    global_step = 0
    epochs_trained = 0
    ...
    model.zero_grad()
    train_iterator = trange(epochs_trained, int(num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0])
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar("loss", (training_loss - logging_loss) / args.logging_steps, global_step)
...
def main():
    ...
    parser.add_argument("--warmup_pct", type=float)
