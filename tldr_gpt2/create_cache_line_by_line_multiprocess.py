"""
Input:
    a single line by line `.txt` file
    each line is raw text

Output:
    a single line by line `.jsonl` file
    each line is pre wordpiece tokenized using GPT2 & converted to indices
"""
...
from examples.run_language_modeling import MODEL_CLASSES, TextDataset, LineByLineTextDataset, load_and_cache_examples, set_seed
...
if __name__ == '__main__':
    ...
    cached_features_file = os.path.join(os.path.dirname(args.data_file), args.model_type + f'_{style}_' + "_cached_lm_" + str(args.block_size) + "_" + os.path.basename(args.data_file).replace('.txt', '.jsonl'))
